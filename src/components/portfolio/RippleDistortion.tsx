...
export function RippleDistortion({
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      const scale = QUALITY_SCALE[quality] ?? QUALITY_SCALE["high"] ?? 1;
    ...
    const loop = (now: number) => {
      ...
      if (geometry.attributes["iOffset"])
        (geometry.attributes["iOffset"] as any).needsUpdate = true;
      if (geometry.attributes["iScale"]) (geometry.attributes["iScale"] as any).needsUpdate = true;
      if (geometry.attributes["iOpacity"])
        (geometry.attributes["iOpacity"] as any).needsUpdate = true;
